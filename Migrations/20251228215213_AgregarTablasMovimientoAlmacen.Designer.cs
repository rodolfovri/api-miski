// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Miski.Infrastructure.Data;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Miski.Api.Migrations
{
    [DbContext(typeof(MiskiDbContext))]
    [Migration("20251228215213_AgregarTablasMovimientoAlmacen")]
    partial class AgregarTablasMovimientoAlmacen
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Miski.Domain.Entities.Accion", b =>
                {
                    b.Property<int>("IdAccion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdAccion"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Icono")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.HasKey("IdAccion");

                    b.ToTable("Accion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Banco", b =>
                {
                    b.Property<int>("IdBanco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdBanco"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdBanco");

                    b.ToTable("Banco", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Cargo", b =>
                {
                    b.Property<int>("IdCargo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCargo"));

                    b.Property<string>("Descripcion")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdCargo");

                    b.ToTable("Cargo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaFAQ", b =>
                {
                    b.Property<int>("IdCategoriaFAQ")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCategoriaFAQ"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("IdCategoriaFAQ");

                    b.ToTable("CategoriaFAQ", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaPersona", b =>
                {
                    b.Property<int>("IdCategoriaPersona")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCategoriaPersona"));

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdCategoriaPersona");

                    b.ToTable("CategoriaPersona", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaProducto", b =>
                {
                    b.Property<int>("IdCategoriaProducto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCategoriaProducto"));

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdCategoriaProducto");

                    b.ToTable("CategoriaProducto", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Compra", b =>
                {
                    b.Property<int>("IdCompra")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCompra"));

                    b.Property<string>("Correlativo")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EsParcial")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EstadoRecepcion")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("FAnulacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FEmision")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FRegistro")
                        .HasColumnType("timestamp without time zone");

                    b.Property<decimal?>("IGV")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<int?>("IdLote")
                        .HasColumnType("integer");

                    b.Property<int>("IdMoneda")
                        .HasColumnType("integer");

                    b.Property<int>("IdNegociacion")
                        .HasColumnType("integer");

                    b.Property<int?>("IdTipoCambio")
                        .HasColumnType("integer");

                    b.Property<int?>("IdUsuarioAnulacion")
                        .HasColumnType("integer");

                    b.Property<decimal?>("MontoTotal")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("MotivoAnulacion")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Observacion")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Serie")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("TipoPago")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdCompra");

                    b.HasIndex("IdLote")
                        .IsUnique();

                    b.HasIndex("IdMoneda");

                    b.HasIndex("IdNegociacion");

                    b.HasIndex("IdTipoCambio");

                    b.HasIndex("IdUsuarioAnulacion");

                    b.ToTable("Compra", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraPago", b =>
                {
                    b.Property<int>("IdCompraPago")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCompraPago"));

                    b.Property<int?>("DiasCredito")
                        .HasColumnType("integer");

                    b.Property<string>("EstadoPago")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("FPago")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdCompra")
                        .HasColumnType("integer");

                    b.Property<decimal?>("MontoAcuenta")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("Observacion")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<decimal?>("Saldo")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("TipoPago")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdCompraPago");

                    b.HasIndex("IdCompra");

                    b.ToTable("CompraPago", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraVehiculo", b =>
                {
                    b.Property<int>("IdCompraVehiculo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCompraVehiculo"));

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("GuiaRemision")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<int>("IdVehiculo")
                        .HasColumnType("integer");

                    b.HasKey("IdCompraVehiculo");

                    b.HasIndex("IdPersona");

                    b.HasIndex("IdVehiculo");

                    b.ToTable("CompraVehiculo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraVehiculoDetalle", b =>
                {
                    b.Property<int>("IdCompraVehiculoDetalle")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCompraVehiculoDetalle"));

                    b.Property<int>("IdCompra")
                        .HasColumnType("integer");

                    b.Property<int>("IdCompraVehiculo")
                        .HasColumnType("integer");

                    b.HasKey("IdCompraVehiculoDetalle");

                    b.HasIndex("IdCompra");

                    b.HasIndex("IdCompraVehiculo");

                    b.ToTable("CompraVehiculoDetalle", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.ConfiguracionGlobal", b =>
                {
                    b.Property<int>("IdConfiguracionGlobal")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdConfiguracionGlobal"));

                    b.Property<string>("Clave")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("EsEditable")
                        .HasColumnType("boolean");

                    b.Property<DateTime?>("FModificacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("TipoDato")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Valor")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.HasKey("IdConfiguracionGlobal");

                    b.HasIndex("Clave")
                        .IsUnique();

                    b.ToTable("ConfiguracionGlobal", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.DetalleMovimientoAlmacen", b =>
                {
                    b.Property<int>("IdDetalleMovimientoAlmacen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdDetalleMovimientoAlmacen"));

                    b.Property<decimal>("Cantidad")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<int?>("IdLote")
                        .HasColumnType("integer");

                    b.Property<int>("IdMovimientoAlmacen")
                        .HasColumnType("integer");

                    b.Property<int>("IdVariedadProducto")
                        .HasColumnType("integer");

                    b.Property<int>("NumeroSacos")
                        .HasColumnType("integer");

                    b.HasKey("IdDetalleMovimientoAlmacen");

                    b.HasIndex("IdLote");

                    b.HasIndex("IdMovimientoAlmacen");

                    b.HasIndex("IdVariedadProducto");

                    b.ToTable("DetalleMovimientoAlmacen", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.DispositivoPersona", b =>
                {
                    b.Property<int>("IdDispositivoPersona")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdDispositivoPersona"));

                    b.Property<bool>("Activo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("DeviceId")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateTime?>("FUltimaActividad")
                        .HasColumnType("timestamp without time zone");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<string>("ModeloDispositivo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("SistemaOperativo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("VersionApp")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdDispositivoPersona");

                    b.HasIndex("DeviceId")
                        .IsUnique()
                        .HasDatabaseName("IX_DispositivoPersona_DeviceId");

                    b.HasIndex("IdPersona")
                        .HasDatabaseName("IX_DispositivoPersona_IdPersona");

                    b.HasIndex("IdPersona", "Activo")
                        .HasDatabaseName("IX_DispositivoPersona_IdPersona_Activo");

                    b.ToTable("DispositivoPersona", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.FAQ", b =>
                {
                    b.Property<int>("IdFAQ")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdFAQ"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdCategoriaFAQ")
                        .HasColumnType("integer");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.Property<string>("Pregunta")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Respuesta")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.HasKey("IdFAQ");

                    b.HasIndex("IdCategoriaFAQ");

                    b.ToTable("FAQ", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.LlegadaPlanta", b =>
                {
                    b.Property<int>("IdLlegadaPlanta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdLlegadaPlanta"));

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("FLlegada")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdCompra")
                        .HasColumnType("integer");

                    b.Property<int>("IdLote")
                        .HasColumnType("integer");

                    b.Property<int>("IdUbicacion")
                        .HasColumnType("integer");

                    b.Property<int>("IdUsuario")
                        .HasColumnType("integer");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<double>("PesoRecibido")
                        .HasPrecision(18, 2)
                        .HasColumnType("double precision");

                    b.Property<double>("SacosRecibidos")
                        .HasPrecision(18, 2)
                        .HasColumnType("double precision");

                    b.HasKey("IdLlegadaPlanta");

                    b.HasIndex("IdCompra");

                    b.HasIndex("IdLote");

                    b.HasIndex("IdUbicacion");

                    b.HasIndex("IdUsuario");

                    b.ToTable("LlegadaPlanta", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Lote", b =>
                {
                    b.Property<int>("IdLote")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdLote"));

                    b.Property<string>("Codigo")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<decimal?>("Comision")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("Observacion")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<decimal>("Peso")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<int>("Sacos")
                        .HasColumnType("integer");

                    b.HasKey("IdLote");

                    b.ToTable("Lote", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Modulo", b =>
                {
                    b.Property<int>("IdModulo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdModulo"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Icono")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.Property<string>("Ruta")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("TipoPlataforma")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.HasKey("IdModulo");

                    b.ToTable("Modulo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Moneda", b =>
                {
                    b.Property<int>("IdMoneda")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdMoneda"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Simbolo")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.HasKey("IdMoneda");

                    b.ToTable("Moneda", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.MovimientoAlmacen", b =>
                {
                    b.Property<int>("IdMovimientoAlmacen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdMovimientoAlmacen"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("FAnulacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int?>("IdLlegadaPlanta")
                        .HasColumnType("integer");

                    b.Property<int>("IdTipoMovimiento")
                        .HasColumnType("integer");

                    b.Property<int>("IdUbicacion")
                        .HasColumnType("integer");

                    b.Property<int>("IdUsuario")
                        .HasColumnType("integer");

                    b.Property<int?>("IdUsuarioAnulacion")
                        .HasColumnType("integer");

                    b.Property<string>("MotivoAnulacion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("TipoStock")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)");

                    b.HasKey("IdMovimientoAlmacen");

                    b.HasIndex("IdLlegadaPlanta");

                    b.HasIndex("IdTipoMovimiento");

                    b.HasIndex("IdUbicacion");

                    b.HasIndex("IdUsuario");

                    b.HasIndex("IdUsuarioAnulacion");

                    b.ToTable("MovimientoAlmacen", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Negociacion", b =>
                {
                    b.Property<int>("IdNegociacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdNegociacion"));

                    b.Property<int?>("AprobadaEvidenciasPorIngeniero")
                        .HasColumnType("integer");

                    b.Property<int?>("AprobadaPorContadora")
                        .HasColumnType("integer");

                    b.Property<int?>("AprobadaPorIngeniero")
                        .HasColumnType("integer");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EstadoAprobacionContadora")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EstadoAprobacionIngeniero")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EstadoAprobacionIngenieroEvidencias")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("EvidenciaVideo")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<DateTime?>("FAdelanto")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FAnulacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FAprobacionContadora")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FAprobacionIngeniero")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FAprobacionIngenieroEvidencias")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FPagoTotal")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FRechazoContadora")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FRechazoIngeniero")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime?>("FRechazoIngenieroEvidencias")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FRegistro")
                        .HasColumnType("timestamp without time zone");

                    b.Property<string>("FotoDniFrontal")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("FotoDniPosterior")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int?>("IdBanco")
                        .HasColumnType("integer");

                    b.Property<int>("IdComisionista")
                        .HasColumnType("integer");

                    b.Property<int?>("IdPersonaProveedor")
                        .HasColumnType("integer");

                    b.Property<int?>("IdTipoDocumento")
                        .HasColumnType("integer");

                    b.Property<int?>("IdUsuarioAnulacion")
                        .HasColumnType("integer");

                    b.Property<int?>("IdVariedadProducto")
                        .HasColumnType("integer");

                    b.Property<decimal?>("MontoAdelanto")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<decimal?>("MontoTotalPago")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("MotivoAnulacion")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("NroCuentaBancaria")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("NroDocumentoProveedor")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Observacion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<decimal?>("PesoPorSaco")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<decimal?>("PesoTotal")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<decimal>("PrecioUnitario")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<string>("PrimeraEvidenciaFoto")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int?>("ProveedorIdPersona")
                        .HasColumnType("integer");

                    b.Property<int?>("RechazadaEvidenciasPorIngeniero")
                        .HasColumnType("integer");

                    b.Property<int?>("RechazadoPorContadora")
                        .HasColumnType("integer");

                    b.Property<int?>("RechazadoPorIngeniero")
                        .HasColumnType("integer");

                    b.Property<int?>("SacosTotales")
                        .HasColumnType("integer");

                    b.Property<string>("SegundaEvidenciaFoto")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("TerceraEvidenciaFoto")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("TipoCalidad")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdNegociacion");

                    b.HasIndex("AprobadaEvidenciasPorIngeniero");

                    b.HasIndex("AprobadaPorContadora");

                    b.HasIndex("AprobadaPorIngeniero");

                    b.HasIndex("IdBanco");

                    b.HasIndex("IdComisionista");

                    b.HasIndex("IdPersonaProveedor");

                    b.HasIndex("IdTipoDocumento");

                    b.HasIndex("IdUsuarioAnulacion");

                    b.HasIndex("IdVariedadProducto");

                    b.HasIndex("ProveedorIdPersona");

                    b.HasIndex("RechazadaEvidenciasPorIngeniero");

                    b.HasIndex("RechazadoPorContadora");

                    b.HasIndex("RechazadoPorIngeniero");

                    b.ToTable("Negociacion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.PermisoRol", b =>
                {
                    b.Property<int>("IdPermisoRol")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPermisoRol"));

                    b.Property<int?>("IdModulo")
                        .HasColumnType("integer");

                    b.Property<int>("IdRol")
                        .HasColumnType("integer");

                    b.Property<int?>("IdSubModulo")
                        .HasColumnType("integer");

                    b.Property<int?>("IdSubModuloDetalle")
                        .HasColumnType("integer");

                    b.Property<bool>("TieneAcceso")
                        .HasColumnType("boolean");

                    b.HasKey("IdPermisoRol");

                    b.HasIndex("IdModulo");

                    b.HasIndex("IdRol");

                    b.HasIndex("IdSubModulo");

                    b.HasIndex("IdSubModuloDetalle");

                    b.ToTable("PermisoRol", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.PermisoRolAccion", b =>
                {
                    b.Property<int>("IdPermisoRolAccion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPermisoRolAccion"));

                    b.Property<bool>("Habilitado")
                        .HasColumnType("boolean");

                    b.Property<int>("IdAccion")
                        .HasColumnType("integer");

                    b.Property<int>("IdPermisoRol")
                        .HasColumnType("integer");

                    b.HasKey("IdPermisoRolAccion");

                    b.HasIndex("IdAccion");

                    b.HasIndex("IdPermisoRol");

                    b.ToTable("PermisoRolAccion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Persona", b =>
                {
                    b.Property<int>("IdPersona")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPersona"));

                    b.Property<string>("Apellidos")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Direccion")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdTipoDocumento")
                        .HasColumnType("integer");

                    b.Property<string>("Nombres")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("NumeroDocumento")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Telefono")
                        .HasMaxLength(15)
                        .HasColumnType("character varying(15)");

                    b.HasKey("IdPersona");

                    b.HasIndex("IdTipoDocumento");

                    b.ToTable("Persona", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaCargo", b =>
                {
                    b.Property<int>("IdPersonaCargo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPersonaCargo"));

                    b.Property<bool>("EsActual")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("FechaFin")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FechaInicio")
                        .HasColumnType("timestamp without time zone");

                    b.Property<int>("IdCargo")
                        .HasColumnType("integer");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<string>("MotivoRevocacion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("ObservacionAsignacion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.HasKey("IdPersonaCargo");

                    b.HasIndex("IdCargo");

                    b.HasIndex("IdPersona");

                    b.ToTable("PersonaCargo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaCategoria", b =>
                {
                    b.Property<int>("IdPersonaCategoria")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPersonaCategoria"));

                    b.Property<int>("IdCategoria")
                        .HasColumnType("integer");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.HasKey("IdPersonaCategoria");

                    b.HasIndex("IdCategoria");

                    b.HasIndex("IdPersona");

                    b.ToTable("PersonaCategoria", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaUbicacion", b =>
                {
                    b.Property<int>("IdPersonaUbicacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdPersonaUbicacion"));

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<int>("IdUbicacion")
                        .HasColumnType("integer");

                    b.HasKey("IdPersonaUbicacion");

                    b.HasIndex("IdPersona");

                    b.HasIndex("IdUbicacion");

                    b.ToTable("PersonaUbicacion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Producto", b =>
                {
                    b.Property<int>("IdProducto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdProducto"));

                    b.Property<string>("Descripcion")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("FichaTecnica")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int>("IdCategoriaProducto")
                        .HasColumnType("integer");

                    b.Property<string>("Imagen")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("IdProducto");

                    b.HasIndex("IdCategoriaProducto");

                    b.ToTable("Producto", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Rol", b =>
                {
                    b.Property<int>("IdRol")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdRol"));

                    b.Property<string>("Descripcion")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("TipoPlataforma")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdRol");

                    b.ToTable("Rol", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Stock", b =>
                {
                    b.Property<int>("IdStock")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdStock"));

                    b.Property<decimal?>("CantidadKg")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)");

                    b.Property<int?>("CantidadSacos")
                        .HasColumnType("integer");

                    b.Property<int>("IdPlanta")
                        .HasColumnType("integer");

                    b.Property<int>("IdVariedadProducto")
                        .HasColumnType("integer");

                    b.Property<string>("TipoStock")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)");

                    b.HasKey("IdStock");

                    b.HasIndex("IdPlanta");

                    b.HasIndex("IdVariedadProducto");

                    b.ToTable("Stock", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModulo", b =>
                {
                    b.Property<int>("IdSubModulo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdSubModulo"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Icono")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("IdModulo")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.Property<string>("Ruta")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("TieneDetalles")
                        .HasColumnType("boolean");

                    b.HasKey("IdSubModulo");

                    b.HasIndex("IdModulo");

                    b.ToTable("SubModulo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloAccion", b =>
                {
                    b.Property<int>("IdSubModuloAccion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdSubModuloAccion"));

                    b.Property<bool>("Habilitado")
                        .HasColumnType("boolean");

                    b.Property<int>("IdAccion")
                        .HasColumnType("integer");

                    b.Property<int>("IdSubModulo")
                        .HasColumnType("integer");

                    b.HasKey("IdSubModuloAccion");

                    b.HasIndex("IdAccion");

                    b.HasIndex("IdSubModulo");

                    b.ToTable("SubModuloAccion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloDetalle", b =>
                {
                    b.Property<int>("IdSubModuloDetalle")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdSubModuloDetalle"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Icono")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("IdSubModulo")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("Orden")
                        .HasColumnType("integer");

                    b.Property<string>("Ruta")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("IdSubModuloDetalle");

                    b.HasIndex("IdSubModulo");

                    b.ToTable("SubModuloDetalle", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloDetalleAccion", b =>
                {
                    b.Property<int>("IdSubModuloDetalleAccion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdSubModuloDetalleAccion"));

                    b.Property<bool>("Habilitado")
                        .HasColumnType("boolean");

                    b.Property<int>("IdAccion")
                        .HasColumnType("integer");

                    b.Property<int>("IdSubModuloDetalle")
                        .HasColumnType("integer");

                    b.HasKey("IdSubModuloDetalleAccion");

                    b.HasIndex("IdAccion");

                    b.HasIndex("IdSubModuloDetalle");

                    b.ToTable("SubModuloDetalleAccion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoCalidadProducto", b =>
                {
                    b.Property<int>("IdTipoCalidadProducto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdTipoCalidadProducto"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdTipoCalidadProducto");

                    b.HasIndex("IdProducto");

                    b.ToTable("TipoCalidadProducto", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoCambio", b =>
                {
                    b.Property<int>("IdTipoCambio")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdTipoCambio"));

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdMoneda")
                        .HasColumnType("integer");

                    b.Property<int>("IdUsuario")
                        .HasColumnType("integer");

                    b.Property<decimal>("ValorCompra")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.Property<decimal>("ValorVenta")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)");

                    b.HasKey("IdTipoCambio");

                    b.HasIndex("IdMoneda");

                    b.HasIndex("IdUsuario");

                    b.ToTable("TipoCambio", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoDocumento", b =>
                {
                    b.Property<int>("IdTipoDocumento")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdTipoDocumento"));

                    b.Property<int?>("LongitudMax")
                        .HasColumnType("integer");

                    b.Property<int?>("LongitudMin")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdTipoDocumento");

                    b.ToTable("TipoDocumento", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoMovimiento", b =>
                {
                    b.Property<int>("IdTipoMovimiento")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdTipoMovimiento"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("TipoOperacion")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdTipoMovimiento");

                    b.ToTable("TipoMovimiento", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.TrackingPersona", b =>
                {
                    b.Property<int>("IdTracking")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdTracking"));

                    b.Property<bool>("EsActual")
                        .HasColumnType("boolean");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("ACTIVO");

                    b.Property<DateTime?>("FActualizacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<string>("Latitud")
                        .IsRequired()
                        .HasPrecision(10, 7)
                        .HasColumnType("text");

                    b.Property<string>("Longitud")
                        .IsRequired()
                        .HasPrecision(10, 7)
                        .HasColumnType("text");

                    b.Property<decimal?>("Precision")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<decimal?>("Velocidad")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.HasKey("IdTracking");

                    b.HasIndex("FRegistro")
                        .HasDatabaseName("IX_TrackingPersona_FRegistro");

                    b.HasIndex("IdPersona")
                        .HasDatabaseName("IX_TrackingPersona_IdPersona");

                    b.HasIndex("IdPersona", "EsActual")
                        .HasDatabaseName("IX_TrackingPersona_IdPersona_EsActual");

                    b.ToTable("TrackingPersona", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Ubicacion", b =>
                {
                    b.Property<int>("IdUbicacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdUbicacion"));

                    b.Property<string>("CodigoSenasa")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("ComprobantePdf")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("DomicilioLegal")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("GiroEstablecimiento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int?>("IdUsuario")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("NumeroRuc")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("RazonSocial")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Tipo")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdUbicacion");

                    b.HasIndex("IdUsuario");

                    b.ToTable("Ubicacion", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.UnidadMedida", b =>
                {
                    b.Property<int>("IdUnidadMedida")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdUnidadMedida"));

                    b.Property<string>("Abreviatura")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdUnidadMedida");

                    b.ToTable("UnidadMedida", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Usuario", b =>
                {
                    b.Property<int>("IdUsuario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdUsuario"));

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int?>("IdPersona")
                        .HasColumnType("integer");

                    b.Property<byte[]>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("bytea");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("IdUsuario");

                    b.HasIndex("IdPersona")
                        .IsUnique();

                    b.HasIndex("Username")
                        .IsUnique();

                    b.ToTable("Usuario", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.UsuarioRol", b =>
                {
                    b.Property<int>("IdUsuarioRol")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdUsuarioRol"));

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<int>("IdRol")
                        .HasColumnType("integer");

                    b.Property<int>("IdUsuario")
                        .HasColumnType("integer");

                    b.HasKey("IdUsuarioRol");

                    b.HasIndex("IdRol");

                    b.HasIndex("IdUsuario");

                    b.ToTable("UsuarioRol", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.VariedadProducto", b =>
                {
                    b.Property<int>("IdVariedadProducto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdVariedadProducto"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FRegistro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp without time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("FichaTecnica")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int>("IdProducto")
                        .HasColumnType("integer");

                    b.Property<int>("IdUnidadMedida")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("IdVariedadProducto");

                    b.HasIndex("IdProducto");

                    b.HasIndex("IdUnidadMedida");

                    b.ToTable("VariedadProducto", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Vehiculo", b =>
                {
                    b.Property<int>("IdVehiculo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdVehiculo"));

                    b.Property<string>("Estado")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Marca")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Modelo")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Placa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("IdVehiculo");

                    b.ToTable("Vehiculo", (string)null);
                });

            modelBuilder.Entity("Miski.Domain.Entities.Compra", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Lote", "Lote")
                        .WithOne("Compra")
                        .HasForeignKey("Miski.Domain.Entities.Compra", "IdLote")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Compra_Lote");

                    b.HasOne("Miski.Domain.Entities.Moneda", "Moneda")
                        .WithMany("Compras")
                        .HasForeignKey("IdMoneda")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Compra_Moneda");

                    b.HasOne("Miski.Domain.Entities.Negociacion", "Negociacion")
                        .WithMany("Compras")
                        .HasForeignKey("IdNegociacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Compra_Negociacion");

                    b.HasOne("Miski.Domain.Entities.TipoCambio", "TipoCambio")
                        .WithMany("Compras")
                        .HasForeignKey("IdTipoCambio")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Compra_TipoCambio");

                    b.HasOne("Miski.Domain.Entities.Usuario", "UsuarioAnulacion")
                        .WithMany()
                        .HasForeignKey("IdUsuarioAnulacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Compra_UsuarioAnulacion");

                    b.Navigation("Lote");

                    b.Navigation("Moneda");

                    b.Navigation("Negociacion");

                    b.Navigation("TipoCambio");

                    b.Navigation("UsuarioAnulacion");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraPago", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Compra", "Compra")
                        .WithMany("CompraPagos")
                        .HasForeignKey("IdCompra")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CompraPago_Compra");

                    b.Navigation("Compra");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraVehiculo", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("CompraVehiculos")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CompraVehiculo_Persona");

                    b.HasOne("Miski.Domain.Entities.Vehiculo", "Vehiculo")
                        .WithMany("CompraVehiculos")
                        .HasForeignKey("IdVehiculo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CompraVehiculo_Vehiculo");

                    b.Navigation("Persona");

                    b.Navigation("Vehiculo");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraVehiculoDetalle", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Compra", "Compra")
                        .WithMany("CompraVehiculoDetalles")
                        .HasForeignKey("IdCompra")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CompraVehiculoDetalle_Compra");

                    b.HasOne("Miski.Domain.Entities.CompraVehiculo", "CompraVehiculo")
                        .WithMany("CompraVehiculoDetalles")
                        .HasForeignKey("IdCompraVehiculo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_CompraVehiculoDetalle_CompraVehiculo");

                    b.Navigation("Compra");

                    b.Navigation("CompraVehiculo");
                });

            modelBuilder.Entity("Miski.Domain.Entities.DetalleMovimientoAlmacen", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Lote", "Lote")
                        .WithMany("DetallesMovimientoAlmacen")
                        .HasForeignKey("IdLote")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_DetalleMovimientoAlmacen_Lote");

                    b.HasOne("Miski.Domain.Entities.MovimientoAlmacen", "MovimientoAlmacen")
                        .WithMany("DetallesMovimientoAlmacen")
                        .HasForeignKey("IdMovimientoAlmacen")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_DetalleMovimientoAlmacen_MovimientoAlmacen");

                    b.HasOne("Miski.Domain.Entities.VariedadProducto", "VariedadProducto")
                        .WithMany("DetallesMovimientoAlmacen")
                        .HasForeignKey("IdVariedadProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_DetalleMovimientoAlmacen_VariedadProducto");

                    b.Navigation("Lote");

                    b.Navigation("MovimientoAlmacen");

                    b.Navigation("VariedadProducto");
                });

            modelBuilder.Entity("Miski.Domain.Entities.DispositivoPersona", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("DispositivoPersonas")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_DispositivoPersona_Persona");

                    b.Navigation("Persona");
                });

            modelBuilder.Entity("Miski.Domain.Entities.FAQ", b =>
                {
                    b.HasOne("Miski.Domain.Entities.CategoriaFAQ", "CategoriaFAQ")
                        .WithMany("FAQs")
                        .HasForeignKey("IdCategoriaFAQ")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_FAQ_CategoriaFAQ");

                    b.Navigation("CategoriaFAQ");
                });

            modelBuilder.Entity("Miski.Domain.Entities.LlegadaPlanta", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Compra", "Compra")
                        .WithMany("LlegadasPlanta")
                        .HasForeignKey("IdCompra")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_LlegadaPlanta_Compra");

                    b.HasOne("Miski.Domain.Entities.Lote", "Lote")
                        .WithMany("LlegadasPlanta")
                        .HasForeignKey("IdLote")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_LlegadaPlanta_Lote");

                    b.HasOne("Miski.Domain.Entities.Ubicacion", "Ubicacion")
                        .WithMany("LlegadasPlanta")
                        .HasForeignKey("IdUbicacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_LlegadaPlanta_Ubicacion");

                    b.HasOne("Miski.Domain.Entities.Usuario", "Usuario")
                        .WithMany("LlegadasPlanta")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_LlegadaPlanta_Ingeniero");

                    b.Navigation("Compra");

                    b.Navigation("Lote");

                    b.Navigation("Ubicacion");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Miski.Domain.Entities.MovimientoAlmacen", b =>
                {
                    b.HasOne("Miski.Domain.Entities.LlegadaPlanta", "LlegadaPlanta")
                        .WithMany("MovimientosAlmacen")
                        .HasForeignKey("IdLlegadaPlanta")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_MovimientoAlmacen_LlegadaPlanta");

                    b.HasOne("Miski.Domain.Entities.TipoMovimiento", "TipoMovimiento")
                        .WithMany("MovimientosAlmacen")
                        .HasForeignKey("IdTipoMovimiento")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_MovimientoAlmacen_TipoMovimiento");

                    b.HasOne("Miski.Domain.Entities.Ubicacion", "Ubicacion")
                        .WithMany("MovimientosAlmacen")
                        .HasForeignKey("IdUbicacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_MovimientoAlmacen_Ubicacion");

                    b.HasOne("Miski.Domain.Entities.Usuario", "Usuario")
                        .WithMany("MovimientosAlmacen")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_MovimientoAlmacen_Usuario");

                    b.HasOne("Miski.Domain.Entities.Usuario", "UsuarioAnulacion")
                        .WithMany()
                        .HasForeignKey("IdUsuarioAnulacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_MovimientoAlmacen_UsuarioAnulacion");

                    b.Navigation("LlegadaPlanta");

                    b.Navigation("TipoMovimiento");

                    b.Navigation("Ubicacion");

                    b.Navigation("Usuario");

                    b.Navigation("UsuarioAnulacion");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Negociacion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Usuario", "AprobadaEvidenciasPorUsuarioIngeniero")
                        .WithMany()
                        .HasForeignKey("AprobadaEvidenciasPorIngeniero")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_AprobadaEvidenciasPorIngeniero");

                    b.HasOne("Miski.Domain.Entities.Usuario", "AprobadaPorUsuarioContadora")
                        .WithMany()
                        .HasForeignKey("AprobadaPorContadora")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_AprobadaPorContadora");

                    b.HasOne("Miski.Domain.Entities.Usuario", "AprobadaPorUsuarioIngeniero")
                        .WithMany()
                        .HasForeignKey("AprobadaPorIngeniero")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_AprobadaPorIngeniero");

                    b.HasOne("Miski.Domain.Entities.Banco", "Banco")
                        .WithMany("Negociaciones")
                        .HasForeignKey("IdBanco")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_Banco");

                    b.HasOne("Miski.Domain.Entities.Usuario", "Comisionista")
                        .WithMany("NegociacionesComisionista")
                        .HasForeignKey("IdComisionista")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Negociacion_Comisionista");

                    b.HasOne("Miski.Domain.Entities.Persona", "PersonaProveedor")
                        .WithMany("NegociacionesPersonaProveedor")
                        .HasForeignKey("IdPersonaProveedor")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_PersonaProveedor");

                    b.HasOne("Miski.Domain.Entities.TipoDocumento", "TipoDocumento")
                        .WithMany("Negociaciones")
                        .HasForeignKey("IdTipoDocumento")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_TipoDocumento");

                    b.HasOne("Miski.Domain.Entities.Usuario", "UsuarioAnulacion")
                        .WithMany()
                        .HasForeignKey("IdUsuarioAnulacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_UsuarioAnulacion");

                    b.HasOne("Miski.Domain.Entities.VariedadProducto", "VariedadProducto")
                        .WithMany("Negociaciones")
                        .HasForeignKey("IdVariedadProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_VariedadProducto");

                    b.HasOne("Miski.Domain.Entities.Persona", "Proveedor")
                        .WithMany()
                        .HasForeignKey("ProveedorIdPersona");

                    b.HasOne("Miski.Domain.Entities.Usuario", "RechazadaEvidenciasPorUsuarioIngeniero")
                        .WithMany()
                        .HasForeignKey("RechazadaEvidenciasPorIngeniero")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_RechazadaEvidenciasPorIngeniero");

                    b.HasOne("Miski.Domain.Entities.Usuario", "RechazadoPorUsuarioContadora")
                        .WithMany()
                        .HasForeignKey("RechazadoPorContadora")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_RechazadoPorContadora");

                    b.HasOne("Miski.Domain.Entities.Usuario", "RechazadoPorUsuarioIngeniero")
                        .WithMany()
                        .HasForeignKey("RechazadoPorIngeniero")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Negociacion_RechazadoPorIngeniero");

                    b.Navigation("AprobadaEvidenciasPorUsuarioIngeniero");

                    b.Navigation("AprobadaPorUsuarioContadora");

                    b.Navigation("AprobadaPorUsuarioIngeniero");

                    b.Navigation("Banco");

                    b.Navigation("Comisionista");

                    b.Navigation("PersonaProveedor");

                    b.Navigation("Proveedor");

                    b.Navigation("RechazadaEvidenciasPorUsuarioIngeniero");

                    b.Navigation("RechazadoPorUsuarioContadora");

                    b.Navigation("RechazadoPorUsuarioIngeniero");

                    b.Navigation("TipoDocumento");

                    b.Navigation("UsuarioAnulacion");

                    b.Navigation("VariedadProducto");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PermisoRol", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Modulo", "Modulo")
                        .WithMany("PermisoRoles")
                        .HasForeignKey("IdModulo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_PermisoRol_Modulo");

                    b.HasOne("Miski.Domain.Entities.Rol", "Rol")
                        .WithMany("PermisoRoles")
                        .HasForeignKey("IdRol")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PermisoRol_Rol");

                    b.HasOne("Miski.Domain.Entities.SubModulo", "SubModulo")
                        .WithMany("PermisoRoles")
                        .HasForeignKey("IdSubModulo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_PermisoRol_SubModulo");

                    b.HasOne("Miski.Domain.Entities.SubModuloDetalle", "SubModuloDetalle")
                        .WithMany("PermisoRoles")
                        .HasForeignKey("IdSubModuloDetalle")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_PermisoRol_SubModuloDetalle");

                    b.Navigation("Modulo");

                    b.Navigation("Rol");

                    b.Navigation("SubModulo");

                    b.Navigation("SubModuloDetalle");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PermisoRolAccion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Accion", "Accion")
                        .WithMany("PermisoRolAcciones")
                        .HasForeignKey("IdAccion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PermisoRolAccion_Accion");

                    b.HasOne("Miski.Domain.Entities.PermisoRol", "PermisoRol")
                        .WithMany("PermisoRolAcciones")
                        .HasForeignKey("IdPermisoRol")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PermisoRolAccion_PermisoRol");

                    b.Navigation("Accion");

                    b.Navigation("PermisoRol");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Persona", b =>
                {
                    b.HasOne("Miski.Domain.Entities.TipoDocumento", "TipoDocumento")
                        .WithMany("Personas")
                        .HasForeignKey("IdTipoDocumento")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Persona_TipoDocumento");

                    b.Navigation("TipoDocumento");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaCargo", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Cargo", "Cargo")
                        .WithMany("PersonaCargos")
                        .HasForeignKey("IdCargo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PersonaCargo_Cargo");

                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("PersonaCargos")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PersonaCargo_Persona");

                    b.Navigation("Cargo");

                    b.Navigation("Persona");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaCategoria", b =>
                {
                    b.HasOne("Miski.Domain.Entities.CategoriaPersona", "Categoria")
                        .WithMany("PersonaCategorias")
                        .HasForeignKey("IdCategoria")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PersonaCategoria_Categoria");

                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("PersonaCategorias")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PersonaCategoria_Persona");

                    b.Navigation("Categoria");

                    b.Navigation("Persona");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PersonaUbicacion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("PersonaUbicaciones")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TU_Persona");

                    b.HasOne("Miski.Domain.Entities.Ubicacion", "Ubicacion")
                        .WithMany("PersonaUbicaciones")
                        .HasForeignKey("IdUbicacion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TU_Ubicacion");

                    b.Navigation("Persona");

                    b.Navigation("Ubicacion");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Producto", b =>
                {
                    b.HasOne("Miski.Domain.Entities.CategoriaProducto", "CategoriaProducto")
                        .WithMany("Productos")
                        .HasForeignKey("IdCategoriaProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Producto_CategoriaProducto");

                    b.Navigation("CategoriaProducto");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Stock", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Ubicacion", "Planta")
                        .WithMany("Stocks")
                        .HasForeignKey("IdPlanta")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Stock_Ubicacion");

                    b.HasOne("Miski.Domain.Entities.VariedadProducto", "VariedadProducto")
                        .WithMany("Stocks")
                        .HasForeignKey("IdVariedadProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_Stock_VariedadProducto");

                    b.Navigation("Planta");

                    b.Navigation("VariedadProducto");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModulo", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Modulo", "Modulo")
                        .WithMany("SubModulos")
                        .HasForeignKey("IdModulo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModulo_Modulo");

                    b.Navigation("Modulo");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloAccion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Accion", "Accion")
                        .WithMany("SubModuloAcciones")
                        .HasForeignKey("IdAccion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModuloAccion_Accion");

                    b.HasOne("Miski.Domain.Entities.SubModulo", "SubModulo")
                        .WithMany("SubModuloAcciones")
                        .HasForeignKey("IdSubModulo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModuloAccion_SubModulo");

                    b.Navigation("Accion");

                    b.Navigation("SubModulo");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloDetalle", b =>
                {
                    b.HasOne("Miski.Domain.Entities.SubModulo", "SubModulo")
                        .WithMany("SubModuloDetalles")
                        .HasForeignKey("IdSubModulo")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModuloDetalle_SubModulo");

                    b.Navigation("SubModulo");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloDetalleAccion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Accion", "Accion")
                        .WithMany("SubModuloDetalleAcciones")
                        .HasForeignKey("IdAccion")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModuloDetalleAccion_Accion");

                    b.HasOne("Miski.Domain.Entities.SubModuloDetalle", "SubModuloDetalle")
                        .WithMany("SubModuloDetalleAcciones")
                        .HasForeignKey("IdSubModuloDetalle")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SubModuloDetalleAccion_SubModuloDetalle");

                    b.Navigation("Accion");

                    b.Navigation("SubModuloDetalle");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoCalidadProducto", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Producto", "Producto")
                        .WithMany("TipoCalidadProductos")
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TipoCalidadProducto_Producto");

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoCambio", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Moneda", "Moneda")
                        .WithMany("TipoCambios")
                        .HasForeignKey("IdMoneda")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TipoCambio_Moneda");

                    b.HasOne("Miski.Domain.Entities.Usuario", "Usuario")
                        .WithMany("TipoCambios")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TipoCambio_Usuario");

                    b.Navigation("Moneda");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TrackingPersona", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithMany("TrackingPersonas")
                        .HasForeignKey("IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_TrackingPersona_Persona");

                    b.Navigation("Persona");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Ubicacion", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Ubicacion_Usuario");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Usuario", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Persona", "Persona")
                        .WithOne("Usuario")
                        .HasForeignKey("Miski.Domain.Entities.Usuario", "IdPersona")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("FK_Usuario_Persona");

                    b.Navigation("Persona");
                });

            modelBuilder.Entity("Miski.Domain.Entities.UsuarioRol", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Rol", "Rol")
                        .WithMany("UsuarioRoles")
                        .HasForeignKey("IdRol")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_UsuarioRol_Rol");

                    b.HasOne("Miski.Domain.Entities.Usuario", "Usuario")
                        .WithMany("UsuarioRoles")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_UsuarioRol_Usuario");

                    b.Navigation("Rol");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Miski.Domain.Entities.VariedadProducto", b =>
                {
                    b.HasOne("Miski.Domain.Entities.Producto", "Producto")
                        .WithMany("VariedadProductos")
                        .HasForeignKey("IdProducto")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_VariedadProducto_Producto");

                    b.HasOne("Miski.Domain.Entities.UnidadMedida", "UnidadMedida")
                        .WithMany("VariedadProductos")
                        .HasForeignKey("IdUnidadMedida")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_VariedadProducto_UnidadMedida");

                    b.Navigation("Producto");

                    b.Navigation("UnidadMedida");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Accion", b =>
                {
                    b.Navigation("PermisoRolAcciones");

                    b.Navigation("SubModuloAcciones");

                    b.Navigation("SubModuloDetalleAcciones");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Banco", b =>
                {
                    b.Navigation("Negociaciones");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Cargo", b =>
                {
                    b.Navigation("PersonaCargos");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaFAQ", b =>
                {
                    b.Navigation("FAQs");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaPersona", b =>
                {
                    b.Navigation("PersonaCategorias");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CategoriaProducto", b =>
                {
                    b.Navigation("Productos");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Compra", b =>
                {
                    b.Navigation("CompraPagos");

                    b.Navigation("CompraVehiculoDetalles");

                    b.Navigation("LlegadasPlanta");
                });

            modelBuilder.Entity("Miski.Domain.Entities.CompraVehiculo", b =>
                {
                    b.Navigation("CompraVehiculoDetalles");
                });

            modelBuilder.Entity("Miski.Domain.Entities.LlegadaPlanta", b =>
                {
                    b.Navigation("MovimientosAlmacen");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Lote", b =>
                {
                    b.Navigation("Compra");

                    b.Navigation("DetallesMovimientoAlmacen");

                    b.Navigation("LlegadasPlanta");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Modulo", b =>
                {
                    b.Navigation("PermisoRoles");

                    b.Navigation("SubModulos");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Moneda", b =>
                {
                    b.Navigation("Compras");

                    b.Navigation("TipoCambios");
                });

            modelBuilder.Entity("Miski.Domain.Entities.MovimientoAlmacen", b =>
                {
                    b.Navigation("DetallesMovimientoAlmacen");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Negociacion", b =>
                {
                    b.Navigation("Compras");
                });

            modelBuilder.Entity("Miski.Domain.Entities.PermisoRol", b =>
                {
                    b.Navigation("PermisoRolAcciones");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Persona", b =>
                {
                    b.Navigation("CompraVehiculos");

                    b.Navigation("DispositivoPersonas");

                    b.Navigation("NegociacionesPersonaProveedor");

                    b.Navigation("PersonaCargos");

                    b.Navigation("PersonaCategorias");

                    b.Navigation("PersonaUbicaciones");

                    b.Navigation("TrackingPersonas");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Producto", b =>
                {
                    b.Navigation("TipoCalidadProductos");

                    b.Navigation("VariedadProductos");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Rol", b =>
                {
                    b.Navigation("PermisoRoles");

                    b.Navigation("UsuarioRoles");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModulo", b =>
                {
                    b.Navigation("PermisoRoles");

                    b.Navigation("SubModuloAcciones");

                    b.Navigation("SubModuloDetalles");
                });

            modelBuilder.Entity("Miski.Domain.Entities.SubModuloDetalle", b =>
                {
                    b.Navigation("PermisoRoles");

                    b.Navigation("SubModuloDetalleAcciones");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoCambio", b =>
                {
                    b.Navigation("Compras");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoDocumento", b =>
                {
                    b.Navigation("Negociaciones");

                    b.Navigation("Personas");
                });

            modelBuilder.Entity("Miski.Domain.Entities.TipoMovimiento", b =>
                {
                    b.Navigation("MovimientosAlmacen");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Ubicacion", b =>
                {
                    b.Navigation("LlegadasPlanta");

                    b.Navigation("MovimientosAlmacen");

                    b.Navigation("PersonaUbicaciones");

                    b.Navigation("Stocks");
                });

            modelBuilder.Entity("Miski.Domain.Entities.UnidadMedida", b =>
                {
                    b.Navigation("VariedadProductos");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Usuario", b =>
                {
                    b.Navigation("LlegadasPlanta");

                    b.Navigation("MovimientosAlmacen");

                    b.Navigation("NegociacionesComisionista");

                    b.Navigation("TipoCambios");

                    b.Navigation("UsuarioRoles");
                });

            modelBuilder.Entity("Miski.Domain.Entities.VariedadProducto", b =>
                {
                    b.Navigation("DetallesMovimientoAlmacen");

                    b.Navigation("Negociaciones");

                    b.Navigation("Stocks");
                });

            modelBuilder.Entity("Miski.Domain.Entities.Vehiculo", b =>
                {
                    b.Navigation("CompraVehiculos");
                });
#pragma warning restore 612, 618
        }
    }
}
