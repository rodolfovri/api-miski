# Módulo de Compras - Asignación de Vehículos

Este módulo gestiona la asignación de compras a vehículos con guías de remisión.

## Funcionalidades principales:
- Asignar múltiples compras a un vehículo
- Registro de guía de remisión
- Consulta de asignaciones con filtros
- Validación de compras no duplicadas

## Controladores:
- `CompraVehiculosController`: CRUD de asignaciones de compras a vehículos
...
### 1. Obtener todas las asignaciones de compras a vehículos
```http
...
{
  "success": true,
  "message": "Asignaciones de compras a vehículos obtenidas exitosamente",
  "data": [
    {
      "idCompraVehiculo": 1,
      "idVehiculo": 1,
...
{
  "success": true,
  "message": "Asignación de compra a vehículo obtenida exitosamente",
  "data": {
    "idCompraVehiculo": 1,
    "idVehiculo": 1,
    "guiaRemision": "GR-2024-001",
...
### 3. Crear asignación de compras a vehículo
```http
...
{
  "success": true,
  "message": "Asignación de compras a vehículo creada exitosamente",
  "data": {
    "idCompraVehiculo": 1,
    "idVehiculo": 1,
    "guiaRemision": "GR-2024-001",
...
### CreateCompraVehiculo:
- ? Vehículo debe existir y ser válido
- ? Guía de remisión es obligatoria (máx. 50 caracteres)
- ? Guía de remisión no puede estar duplicada
...
- ? Todos los IDs de compras deben ser válidos (mayor a 0)

## Entidades Relacionadas:
- **CompraVehiculo** ? Entidad principal (cabecera)
- **CompraVehiculoDetalle** ? Detalle (relación compra-vehículo)
- **Vehiculo** ? Vehículo asignado
- **Compra** ? Compras asignadas al vehículo
...
## Flujo de Asignación de Compras a Vehículo:

```
1. NEGOCIACIÓN APROBADA POR CONTADORA
   ?
2. SE CREA COMPRA AUTOMÁTICAMENTE (Estado: ACTIVO)
   ?
3. ASIGNAR COMPRA(S) A VEHÍCULO
   POST /api/compras/asignacion-vehiculos
   {
     "idVehiculo": 1,
     "guiaRemision": "GR-2024-001",
...
5. SE CREAN CompraVehiculoDetalle (Detalles)
   ...
   ?
6. COMPRAS ASIGNADAS AL VEHÍCULO
   ? Listas para transporte con guía de remisión
```

## Casos de Uso:

### Caso 1: Asignar una sola compra a un vehículo
```http
POST /api/compras/asignacion-vehiculos
{
  "idVehiculo": 1,
...
### Caso 2: Asignar múltiples compras a un vehículo
```http
POST /api/compras/asignacion-vehiculos
{
  "idVehiculo": 2,
...
### Caso 3: Consultar todas las asignaciones de un vehículo
```http
GET /api/compras/asignacion-vehiculos?idVehiculo=1
```

### Caso 4: Buscar por guía de remisión
```http
...
### Error: Compra ya asignada a otro vehículo
```json
{
  "success": false,
  "message": "Validación fallida",
  "errors": {
    "Compra": ["La compra con ID 5 ya está asignada a un vehículo"]
  }
}
```
...
```json
{
  ...
  "message": "Validación fallida",
  "errors": {
    "GuiaRemision": ["La guía de remisión 'GR-2024-001' ya está registrada"]
  }
}
```

### Error: Compra no está en estado ACTIVO
```json
{
  "success": false,
  "message": "Validación fallida",
  "errors": {
    "Compra": ["La compra con ID 5 no está en estado ACTIVO"]
  }
}
```

## Prioridad: ALTA
Las asignaciones de compras a vehículos son esenciales para el control de transporte y trazabilidad.
